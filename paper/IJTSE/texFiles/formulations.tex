...
Based on the postulation of strain-energy function, it follows that the work done on hyperelastic materials in a dynamic process within a time interval $[t_1, t_2]$ is the difference between the strain-energy in the two states. It can also be proven that the work can be written as the integration of the tensor product of conjugate stress-strain rate pairs \cite{Holzapfel}:

\begin{equation}
\begin{split}
\Psi(\F_2) - \Psi(\F_1) &=  \int_{t_1}^{t_2}\bold{S}:\dot{\bold{E}}dt = \int_{t_1}^{t_2}\bold{P}:\dot{\bold{F}}dt \\
				  &=  \int_{t_1}^{t_2}\boldsymbol{\sigma}:\dot{\bold{e}}dt
\end{split}
\end{equation}
where $\bold{P}$ is the first Piola-Kirchhoff (PK1) stress defined as $\bold{P} = J\boldsymbol{\sigma}{\bold{F}}^{-T}$, $\bold{S}$ is the second Piola-Kirchhoff (PK2) stress tensor, $\bold{E}$ is the Green-Lagrange strain tensor defined as $\bold{E} = \frac{1}{2}(\C - \I)$, and $\bold{e}$ is Euler-Almansi strain tensor which is defined as $\bold{e} = \frac{1}{2}( \bold{I} - {\bold{F}}^{-T}{\bold{F}}^{-1} )$.
...
Similarly, the PK1 stress and Cauchy stress can be determined as $\bold{P} = 2\bold{F}\dfrac{\partial \Psi(\bold{C})}{\partial \bold{C}}$ and $\boldsymbol{\sigma} = J^{-1}\bold{F} \dfrac{\partial\Psi(\bold{C})}{\partial \bold{C}} \bold{F}^T$.
...
&=
\mathbb{P} : \overline{\mathbb{C}} : \mathbb{P}^T + \frac{2}{3}\mathrm{Tr}(J^{-2/3}\overline{\bold{S}})\tilde{\mathbb{P}} \\
& - \frac{2}{3}J^{-2/3}(\overline{\bold{C}}^{-1}\otimes\bold{S}_\mathrm{iso} + \bold{S}_\mathrm{iso}\otimes {\overline{\bold{C}}}^{-1})
\end{split}
\label{Ciso} 
\end{align}
\end{subequations}
...
Therefore, once the stress expressions $\PK_\mathrm{vol}$ and $\PK_\mathrm{iso}$ are obtained, we can evaluate the elasticity tensor $\CC$ by substituting Equation \ref{S} into Equation \ref{C}. The stress and elasticity tensors in the current configuration can be obtained easily using push-forward operations as derived in Appendix \ref{updated}.
...
\begin{split}
\mathbb{C}_{\mathrm{vol}} = {} & J^{-1/3}(2J-1)\kappa \overline{\bold{C}}^{-1} \otimes \overline{\bold{C}}^{-1}\\
&- 2J^{-1/3}(J-1)\kappa \overline{\bold{C}}^{-1} \odot \overline{\bold{C}}^{-1}
\end{split}
\label{Cvol12}
\end{align}
\end{subequations} 
...
\end{equation}
%\end{strip}
where the $\partial\Psi_\mathrm{aniso}/\partial{\bar{I}_i}$ and $\partial^2\Psi_\mathrm{aniso}/{\partial{\bar{I}_i}^2}$ ($i = 4, 6$) are given in Equation \ref{convert}.
